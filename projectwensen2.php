...
<script type="text/javascript">
	...
	function skillssearch(search) {
		var val = search;
        var sel = document.getElementById('skilloption');
...
<body onLoad="menu(), selecttab('img2', 'skills', 'searchskills')<?php
	if(isset($_GET["s"])){
...
<div class="content">
	<div class="search shadow">
		...
		<div class="searchresult shadow">
			<div class="title radius"><p>Projecten</p></div>
			<div class="content" id="searchresults">
				 <?php
				     $data = $rs->dataOpvragen("SELECT * FROM `projectsusers`"); 
				     foreach ($data as $rij)	{
				     echo "<div class=\"listitem2 \">" . $rij["project"] . $rij["functie"] . "</div>";
				     }
